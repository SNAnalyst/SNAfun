...
vignette: >
  %\VignetteIndexEntry{Temporal networks exploration and description}
...
For reading active vertex and edge attributes, it is usually easier to
stay inside the `snafun` API than to call the raw
`networkDynamic::*active` functions directly.

-   `snafun::list_vertex_attributes(g, active = TRUE, onset = 5, terminus = 8)`
    List the attributes of the vertices that are active in a specific
    time segment.

-   `snafun::extract_vertex_attribute(g, "attrName", active = TRUE, at = 1)`
    The value for vertex attribute `attrName` in a specific time
    segment.

-   `snafun::list_edge_attributes(g, active = TRUE, onset = 0, terminus = 49)`
    List the attributes of the edge that are active in a specific time
    segment.

-   `snafun::extract_edge_attribute(g, "attrName", active = TRUE, at = 1)`
...
NOTE: The raw `networkDynamic` functions for accessing and setting the
attributes of a `networkDynamic` object are not very user friendly.
Luckily, you can often stay inside the `snafun` API. As long as you want
to access and/or set attributes that are *static*, this works much
easier and uses functions that you have used multiple times already in
this course and should be second nature to you by now.

For `snafun`, it is important to remember the difference between
`active = FALSE` and `active = TRUE` when the input is a
`networkDynamic` object:

-   with the default `active = FALSE`, `snafun` treats the object as an
    ordinary `network` object and returns the attributes as they are
    stored internally. Static attributes use their usual names, while
    dynamic TEAs show up with names such as `gender.active` or
    `weight.active`.

-   with `active = TRUE`, `snafun` uses the `networkDynamic` accessors
    and interprets the attribute over time. In that case you query the
    logical attribute name (for example `"weight"` instead of
    `"weight.active"`) and also provide a time point or interval.
...
# raw stored TEA
snafun::extract_edge_attribute(classroom, "weight.active")

# interpreted value at a specific time point
snafun::extract_edge_attribute(classroom, "weight", active = TRUE, at = 1)
```

## Network measures and descriptives

The `snafun` package adds a few convenience helpers for temporal
descriptives. They operate directly on `networkDynamic` objects:

```{r table_temporal, echo = FALSE, eval = TRUE}
table_temporal
```
...
-   `tsna::tiedDuration(g, mode = "counts")` Computes the total number
...
```{r}
# plot g at 9 equally-spaced time slices
snafun::plot_network_slices(g, number = 9)
```
