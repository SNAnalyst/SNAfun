...
The `snafun` package provides a `plot(x)` function, which allows the user to 
make a quick plot of a network, regardless of whether `x` is a graph of class 
`network` or `igraph`. This function wraps `network::plot.network` and 
`igraph::plot.igraph` and will use the default settings of these functions.

However, one can also use all of the arguments from `network::plot.network` and 
`igraph::plot.igraph` to make the plots nicer when wanted. 
...
The most common use case for the function is to make as quick plot of the network, 
using default settings, as one usually would do in the initial phase of a study. 
For this, `snafun` provides a consistent function name.

The `snafun` package also contains a function to plot centrality scores of
