...
net_edges <- add_edge_attributes(net, edgelist = edges)
expect_equal(igraph::edge_attr(net_edges, "one"), edges$one)
expect_equal(igraph::edge_attr(net_edges, "two"), edges$two)
expect_true(all(c("one", "two") %in% igraph::edge_attr_names(net_edges)))


net_edges <- add_edge_attributes(net, attr_name = "one", edgelist = edges)
expect_equal(igraph::edge_attr(net_edges, "one"), edges$one)
expect_null(igraph::edge_attr(net_edges, "two"))
expect_true(sum(c("one", "two") %in% igraph::edge_attr_names(net_edges)) == 1)


net_edges <- add_edge_attributes(net, attr_name = "two", edgelist = edges)
expect_null(igraph::edge_attr(net_edges, "one"))
expect_equal(igraph::edge_attr(net_edges, "two"), edges$two)
expect_true(sum(c("one", "two") %in% igraph::edge_attr_names(net_edges)) == 1)
...
net_edges_reo <- add_edge_attributes(net, edgelist = edges_reo)
# check that they are still included in the original, correct order
expect_equal(igraph::edge_attr(net_edges_reo, "one"), edges$one)
expect_equal(igraph::edge_attr(net_edges_reo, "two"), edges$two)
expect_true(all(c("one", "two") %in% igraph::edge_attr_names(net_edges_reo)))
...
edges_na[10, "one"] <- NA  # the 10th dyad gets value NA
net_edges_na <- add_edge_attributes(net, edgelist = edges_na)
expect_equal(igraph::edge_attr(net_edges_na, "one"), edges_na$one)
expect_equal(igraph::edge_attr(net_edges_na, "two"), edges_na$two)
expect_true(all(c("one", "two") %in% igraph::edge_attr_names(net_edges_na)))
expect_true(is.na(igraph::edge_attr(net_edges_na, "one")[10]))
...
edges_ord <- edges[order(volgorde), ]
expect_equal(igraph::edge_attr(net_edges, "one"), edges_ord$one)
expect_equal(igraph::edge_attr(net_edges, "two"), edges_ord$two)
expect_true(all(c("one", "two") %in% igraph::edge_attr_names(net_edges)))
...
net_edges <- add_edge_attributes(net, value = val, attr_name = "hiephoi")
expect_equal(igraph::edge_attr(net_edges, "hiephoi"), val)
expect_true("hiephoi" %in% igraph::edge_attr_names(net_edges))
expect_error(add_edge_attributes(net, value = val, attr_name = c("hiephoi", "boe")),
             "'attr_name' and 'value' do not have matching dimensions"
)
...
net_edges <- add_edge_attributes(net, value = val, attr_name = c("one", "two"))
expect_equal(igraph::edge_attr(net_edges, "one"), val[, 1])
expect_equal(igraph::edge_attr(net_edges, "two"), val[, 2])
expect_true(all(c("one", "two") %in% igraph::edge_attr_names(net_edges)))
