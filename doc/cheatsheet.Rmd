...
The `SNA4DS::Ef_int(m)` function estimates the intensity of the predictor's 
effect on the outcome variable in an `ergm` model. Effects can be estimated 
specifying the option `Odds` to print odds ratios or `prob` to print
probabilities.
...
-   Dyadic independent and dyadic dependent terms: We encounter
    the first one when the probability of edge formation is related to nodes
...
-   Exogenous and Endogenous terms: The first one refers to terms using covariates, 
    the second to structural terms.
...
-   Compute probability using the `SNA4DS` function

```{r}

P <- Ef_int(m, type = "probs")
...
network::set.vertex.attribute(BipNet, "vertex.names",  vertex.names)
network::set.vertex.attribute(BipNet, "attrib1",  attrib1)
...
network::set.vertex.attribute(BipNet, "bipratite", value = rep(10, 10), v = 1:10)
...
- `b1nodematch` & `b2nodematch` -- same as `nodematch()` for binary ERGMs
...
Convergence problems? Use exponential down-weighting. AKA "curve" the terms:
...
- `sender(covariate = "MyCov")` -- different from `ergm`, here you can insert an attribute

- `receiver(covariate = "MyCov")` -- different from `ergm`, here you can insert an attribute
